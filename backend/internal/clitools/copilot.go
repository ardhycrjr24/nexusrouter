...
func (t *CopilotTool) DetectStatus(homeDir string) (bool, bool, string, error) {
	...
	for _, e := range entries {
		if name, _ := e["name"].(string); name == "NexusRouter" {
...
func (t *CopilotTool) Configure(homeDir, baseURL, apiKey string, models []string) error {
	...
	// Remove existing NexusRouter entry.
...
// containsNexusRouter reports whether a JSON string contains a NexusRouter marker.
func containsNexusRouter(s string) bool {
	return strings.Contains(s, "NexusRouter") || strings.Contains(s, "nexusrouter")
}
