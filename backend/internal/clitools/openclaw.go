...
func (t *OpenClawTool) Configure(homeDir, baseURL, apiKey string, models []string) error {
	...
	modelsMap["providers"] = map[string]any{"nexusrouter": provider}
	...
	if len(models) > 0 {
		...
		defaults["model"] = map[string]any{"primary": "nexusrouter/" + models[0]}
	...
	if agents, ok := cfg["agents"].(map[string]any); ok {
		if defaults, ok := agents["defaults"].(map[string]any); ok {
			if model, ok := defaults["model"].(map[string]any); ok {
				if primary, ok := model["primary"].(string); ok && containsNexusRouter(primary) {
