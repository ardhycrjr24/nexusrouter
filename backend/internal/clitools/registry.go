...
// tool knows how to detect its installation status, write NexusRouter-specific
...
// only NexusRouter-specific keys are added or removed.
package clitools
...
// Tool describes a CLI tool that can be auto-configured to use NexusRouter.
type Tool interface {
	// ID returns the tool's short identifier (e.g. "claude", "codex").
	ID() string
	// Name returns the human-readable display name.
	Name() string
	// DetectStatus checks whether the tool is installed and whether NexusRouter
	...
	// Configure writes NexusRouter settings into the tool's config file(s).
	Configure(homeDir, baseURL, apiKey string, models []string) error
	// Remove strips NexusRouter settings from the tool's config file(s).
