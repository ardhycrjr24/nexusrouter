...
import "github.com/ardhycrjr24/nexusrouter/backend/internal/core"
...
// dialect NexusRouter does not yet drive natively remain in the catalog for
...
func freeTierProviders() []ProviderSpec {
	return []ProviderSpec{
		...
		{ID: "vllm", DisplayName: "vLLM", Alias: "vllm", Dialect: core.DialectOpenAI,
			...
			Notice:  "Self-hosted vLLM OpenAI-compatible server. Set base URL to your vLLM endpoint; provide an API key only if you started vLLM with --api-key. Requires NEXUSROUTER_SECURITY__ALLOW_PRIVATE_BASE_URL=true when pointing at loopback or LAN hosts."},
