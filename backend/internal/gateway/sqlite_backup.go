...
func (s *Server) adminSQLiteBackup(w http.ResponseWriter, r *http.Request) {
	...
	tmp, err := os.CreateTemp(s.dataDirOrTemp(), "nexusrouter-sqlite-backup-*.db")
	if err != nil {
	...
	filename := "nexusrouter-sqlite-" + time.Now().UTC().Format("20060102-150405") + ".db"
...
func (s *Server) adminSQLiteRestore(w http.ResponseWriter, r *http.Request) {
	...
	tmp, err := os.CreateTemp(s.dataDirOrTemp(), "nexusrouter-sqlite-restore-*.db")
	if err != nil {
