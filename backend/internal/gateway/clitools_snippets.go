...
// given base URL and model. The baseURL is the public NexusRouter endpoint (with
...
model = "%s"
model_provider = "nexusrouter"

[model_providers.nexusrouter]
name = "NexusRouter"
base_url = "%s"
wire_api = "chat"
	...
	snippet := fmt.Sprintf(`# ~/.factory/settings.json
{
  "customModels": [
    {
      "model": "%s",
      "id": "custom:NexusRouter-0",
      "index": 0,
      "baseUrl": "%s",
      "apiKey": "%s",
      "displayName": "NexusRouter",
...
[providers.nexusrouter]
type = "openai-compatible"
base_url = "%s"
auth = "bearer"
api_key_env = "JCODE_NEXUSROUTER_API_KEY"
env_file = "~/.config/jcode/provider-nexusrouter.env"
default_model = "%s"
requires_api_key = true

[[providers.nexusrouter.models]]
id = "%s"
name = "%s"

# ~/.config/jcode/provider-nexusrouter.env
JCODE_NEXUSROUTER_API_KEY="%s"`, baseURL, model, model, model, apiKey)
	return cliToolSnippet{
		ID:           "jcode",
		Name:         "jcode",
		Dialect:      "toml",
