---
name: NexusRouter-chat
description: Chat / code generation via NexusRouter using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through NexusRouter.
---
...
- `POST $NexusRouter_URL/v1/chat/completions` — OpenAI format
- `POST $NexusRouter_URL/v1/messages` — Anthropic format
- `POST $NexusRouter_URL/v1/responses` — OpenAI Responses format

## Discover

```bash
curl $NexusRouter_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$NexusRouter_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $NexusRouter_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.NexusRouter_URL}/v1`, apiKey: process.env.NexusRouter_KEY });
...
```bash
curl -X POST $NexusRouter_URL/v1/messages \
  -H "Authorization: Bearer $NexusRouter_KEY" \
