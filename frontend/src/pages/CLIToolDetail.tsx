...
export function CLIToolDetailPage() {
  ...
  const configureMut = useMutation({
    mutationFn: () =>
      api.cliToolConfigure(toolId!, {
        base_url: baseUrl,
        api_key: apiKey || "sk_NexusRouter",
        models: model ? [model] : undefined,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cli-tools"] });
      toast.success("Tool configured", `${tool?.name} is now routing through NexusRouter. All requests will use the proxy endpoint.`);
    },
  ...
  const removeMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cli-tools"] });
      toast.success("Config removed", `${tool?.name} has been disconnected from NexusRouter and will use its default endpoint.`);
    },
  ...
  const snippetWithVars = tool.snippet
    ...
    .replace(/sk_NexusRouter/g, apiKey || "sk_NexusRouter");
            ...
            <div className="flex items-center gap-2">
              ...
              {keys.data?.keys && keys.data.keys.length > 0 ? (
                <Select
                  ...
                  className="flex-1"
                >
                  <option value="">sk_NexusRouter (default)</option>
