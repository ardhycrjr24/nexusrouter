...
export function Layout() {
  ...
  useEffect(() => {
    ...
    const appName = branding.name || "NexusRouter";
  ...
  return (
    <aside className="flex h-full w-60 shrink-0 flex-col border-r border-[var(--border)] bg-[var(--bg-elevated)]">
      <div className="flex items-center justify-between px-5 py-5">
        <img src={logoSrc} alt={branding.name || "NexusRouter"} className="h-14 w-full object-contain object-left" />
      ...
      >
        <div className="hidden sm:block">
          ...
          <span className="block w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] py-2 pl-9 pr-12 text-sm text-[var(--text-muted)]">
            Search {branding.name || "NexusRouter"}…
          </span>
...
function ProfileMenu() {
  ...
  const name = branding.name || "NexusRouter";
  const tagline = branding.tagline || "AI Gateway";
  const avatarFallback = name.charAt(0).toUpperCase() || "N";
    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <div className="hidden text-left sm:block">
          ...
          <p className="text-xs leading-tight text-[var(--text-muted)]">{tagline}</p>
        </div>
