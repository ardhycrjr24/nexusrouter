...
function AuthGateLogo({ className }: { className?: string }) {
  ...
  const src = data?.logo_url || "/nexusrouter-logo.png";
  return <img src={src} alt={data?.name || "NexusRouter"} className={className} />;
}
    ...
    <div className="flex h-full items-center justify-center px-4">
      <Card className="w-full max-w-sm p-8 shadow-[var(--shadow-pop)]">
        ...
        <p className="mt-4 text-center text-xs text-[var(--text-muted)]">
          ...
          <code className="font-mono">NexusRouter</code>.
        </p>
      </Card>
    </div>
  );
