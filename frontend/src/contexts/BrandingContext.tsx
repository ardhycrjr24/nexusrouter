...
interface BrandingContextValue {
  ...
  faviconSrc: string;
  isLoading: boolean;
  setPreviewBranding?: (preview: BrandingSettings | null) => void;
}

const BrandingContext = createContext<BrandingContextValue>({
  branding: DEFAULT_BRANDING,
  logoSrc: "/nexusrouter-logo.png",
  faviconSrc: "/nexusrouter-favicon.png",
  isLoading: false,
});
...
export function AdminBrandingProvider({ children }: { children: ReactNode }) {
  ...
  const [preview, setPreview] = useState<BrandingSettings | null>(null);

  const branding = preview ?? data ?? DEFAULT_BRANDING;
  const logoSrc = branding.logo_url || "/nexusrouter-logo.png";
  const faviconSrc = branding.favicon_url || "/nexusrouter-favicon.png";
  ...
  return (
    <BrandingContext.Provider value={{ branding, logoSrc, faviconSrc, isLoading, setPreviewBranding: setPreview }}>
      {children}
...
export function PortalBrandingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const title = branding.name || "NexusRouter";
