// Typed client for the NexusRouter admin API. All calls go through the dev-server
...
export async function fetchPortalBranding(): Promise<BrandingSettings> {
  ...
  if (!resp.ok) {
    return { name: "NexusRouter", logo_url: "", favicon_url: "", tagline: "", color_palette: "sage-terra" };
  }
  return resp.json();
}
...
export const api = {
  ...
  createAgent: (input: { name: string; description?: string; system_prompt?: string; model?: string; skills?: string[] }) =>
    request<{ id: string; name: string }>("POST", "/agents", input),
  updateAgent: (id: string, patch: { name?: string; description?: string; system_prompt?: string; model?: string; skills?: string[] }) =>
    request<void>("PATCH", `/agents/${id}`, patch),
