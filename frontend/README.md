# NexusRouter Dashboard

The web control surface for NexusRouter, built with React 19, Vite, TypeScript,
and Tailwind CSS 4.
...
Run the Go backend separately (`cd ../backend && go run ./cmd/NexusRouter`) so the
...
(`/usr/local/share/NexusRouter/frontend/dist` by default) or from the Docker image.
