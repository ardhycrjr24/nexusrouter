...
dev:
	@printf "$(C_BOLD)$(C_CYAN)Starting NexusRouter$(C_RESET) backend (:20180) + dashboard (:5180)…\n"
	@trap 'kill 0' INT TERM EXIT; \
	( cd $(BACKEND_DIR) && go run ./cmd/nexusrouter ) & \
	( \
...
build-backend:
	cd $(BACKEND_DIR) && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.Version=$$(git describe --tags --always --dirty 2>/dev/null || echo dev)" -o ../$(BIN) ./cmd/nexusrouter
...
bootstrap:
	cd $(BACKEND_DIR) && go run ./cmd/nexusrouter -bootstrap

## docker: build the production image.
docker:
	docker build -f deploy/Dockerfile -t nexusrouter:latest .
...
##        Usage: make setup
setup:
	@printf "$(C_BOLD)$(C_CYAN)🚀 NexusRouter$(C_RESET) — one-command setup\n"
