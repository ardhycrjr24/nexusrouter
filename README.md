<p align="center">
  <img src="nexusrouter-logo.png" alt="NexusRouter Logo" width="160">
</p>
<h1 align="center">NexusRouter 🚀</h1>
...
<p align="center">
  <a href="https://github.com/ardhycrjr24/NexusRouter/actions/workflows/ci.yml"><img src="https://github.com/ardhycrjr24/NexusRouter/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  <a href="https://go.dev/"><img src="https://img.shields.io/badge/Go-1.24+-00ADD8?logo=go&logoColor=white" alt="Go 1.24+"></a>
  <a href="https://github.com/ardhycrjr24/NexusRouter/pkgs/container/NexusRouter"><img src="https://img.shields.io/badge/Docker-GHCR-2496ED?logo=docker&logoColor=white" alt="Docker Image"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-yellow.svg" alt="License: MIT"></a>
</p>

<p align="center">
  <img src="assets/NexusRouter-social-banner.png" alt="NexusRouter Dashboard" width="800">
</p>

---

Hey there! 👋 Welcome to **NexusRouter**.
...
## 🌟 Why you'll love NexusRouter

- 🔀 **One Endpoint to Rule Them All:** Your apps only need to speak OpenAI or Anthropic. NexusRouter handles translating your requests to whatever provider you actually want to use behind the scenes.
- 🛡️ **Never Stop Coding:** Hit a rate limit? Provider down? NexusRouter automatically falls back to your backup models so your workflow never gets interrupted.
- 💸 **Save Serious Cash:** 
...
- 🔧 **CLI Tools Auto-Config:** NexusRouter generates ready-to-paste configuration snippets for 11+ coding tools—Claude Code, Cursor, Cline, GitHub Copilot, DeepSeek, KiloCode, and more.
...
<p align="center">
  <img src="assets/NexusRouter-providers-banner.png" alt="Manage AI Providers" width="800">
</p>

<p align="center">
  <img src="assets/NexusRouter-usage-banner.png" alt="Intelligent Routing & Topology" width="800">
</p>
...
```bash
brew tap ardhycrjr24/NexusRouter https://github.com/ardhycrjr24/NexusRouter
brew install NexusRouter
```
...
- Clones the repo to `~/NexusRouter`
- Installs dependencies
- Starts backend on `:20180` and dashboard on `:5180`
- Default dashboard password: `NexusRouter`

> **Already have the repo?** Just run `make setup` in the project root.

**Prefer Docker?** No Go/Node.js needed:
```bash
curl -fsSL https://raw.githubusercontent.com/ardhycrjr24/NexusRouter/main/scripts/install.sh | bash -s -- --docker
```

**VPS / Coolify / Production?**
```bash
git clone https://github.com/ardhycrjr24/NexusRouter.git
cd NexusRouter
cp .env.example .env   # set NexusRouter_MASTER_KEY for production
...
Log in with the default password: `NexusRouter` (it will ask you to change this immediately for security).

You can also create an API key directly from the terminal without the dashboard:
```bash
NexusRouter -bootstrap   # prints a kr_ key once
```
...
Then, in your app, just set the model to `chain:coding` (or just `coding`) and let NexusRouter do the heavy lifting!

## 🔌 What else can it do?
NexusRouter isn't just for chat! It supports everything:
...
Tired of copying API keys? You can connect providers like Claude, GitHub Copilot, Gemini CLI, and more directly from the Connections page using OAuth. Just click, sign in, and NexusRouter handles securely refreshing your tokens in the background!
...
NexusRouter ships a built-in content-safety layer that runs detectors against every request and response. Policies are layered **global → provider → model → chain → API key** and merged at request time, so the most specific override wins.
...
**Observability:** Every decision lands in the **Audit Logs** tab with full findings (entity, score, redaction). The dashboard streams new rows live via SSE. Prometheus exposes `NexusRouter_guardrail_decisions_total{detector,action}` and `NexusRouter_guardrail_eval_seconds{detector}` at `/metrics`.

**Compliance & hardening:**
- **GDPR / data residency:** A per-tenant `allow_external_engines` flag forces all detectors back to their native engines so no data ever leaves the NexusRouter process.
- **Audit retention:** `NexusRouter_GUARDRAILS__AUDIT_RETENTION_DAYS=90` runs an hourly sweeper to drop old rows.
...
Make NexusRouter your own! The branding system lets you customize the entire look and feel of both the admin dashboard and the public-facing Usage Portal.

Customizable settings include:
- **App Name** — Replace "NexusRouter" with your organization's name
...
NexusRouter can generate ready-to-paste configuration snippets for your favorite AI coding tools. Visit the **CLI Tools** page in the dashboard to get the exact config you need for:
...
NexusRouter connects to a massive roster of AI providers out of the box. Here's the full list:
...
## ⚙️ Configuration
By default, NexusRouter uses an embedded SQLite database (zero config required!). If you are deploying it for a team, you can use PostgreSQL. Just copy `config.example.yaml` and run with `-config`, or use environment variables like `NexusRouter_SERVER__PORT=8080`. Docker/Coolify examples live in [deploy/README.md](deploy/README.md).
